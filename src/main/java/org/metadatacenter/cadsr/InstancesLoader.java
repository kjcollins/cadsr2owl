...
import org.metadatacenter.cadsr.cadsr_objects.jaxb.Classification;
import org.metadatacenter.cadsr.cadsr_objects.jaxb.ComponentDataElement;
import org.metadatacenter.cadsr.cadsr_objects.jaxb.Concept;
import org.metadatacenter.cadsr.cadsr_objects.jaxb.DataElement;
import org.metadatacenter.cadsr.cadsr_objects.jaxb.ReferenceDocument;
import org.metadatacenter.cadsr.cadsr_objects.jaxb.AlternateName;
...
import org.metadatacenter.cadsr.cadsr_objects.jaxb.ValueDomainConcept;
...
public class InstancesLoader
{
   ...
   * @param dataElementsList
   * @throws OWLOntologyCreationException, OWLOntologyStorageException, InstanceNotFoundException
   */
  @Nonnull public void createOntologyInstances(@Nonnull OWLOntologyManager manager, @Nonnull File ontFile,
    @Nonnull File populatedOntFile, @Nonnull DataElementsList dataElementsList)
    throws OWLOntologyCreationException, OWLOntologyStorageException, InstanceNotFoundException, NoSuchMethodException,
  ...
  @Nonnull private OWLIndividual createDataElement(@Nonnull OWLOntologyManager manager,
    @Nonnull DataElement dataElement, @Nonnull List<String> typesToExplore)
    throws InstanceNotFoundException, NoSuchMethodException, IllegalAccessException, InvocationTargetException
  {
  ...
  private OWLObjectProperty createObjectPropertyAssertion(@Nonnull OWLOntologyManager manager, @Nonnull IRI ontologyIRI,
    @Nonnull OWLIndividual sourceInd, @Nonnull String propertyName, @Nonnull OWLIndividual targetInd)
    throws InstanceNotFoundException
  {
    OWLDataFactory factory = manager.getOWLDataFactory();
    IRI opIRI = IRI.create(ontologyIRI + "#" + propertyName);
    if (ont.containsObjectPropertyInSignature(opIRI) == false) {
      throw new InstanceNotFoundException("The object property \"" + opIRI + "\" was not found in the ontology");
    }
  ...
  @Nonnull private OWLIndividual createInstanceFromObject(@Nonnull OWLOntologyManager manager, @Nonnull Object obj,
    String idMethodName, @Nonnull List<String> typesToExplore)
    throws InstanceNotFoundException, NoSuchMethodException, InvocationTargetException, IllegalAccessException
  {
    OWLDataFactory factory = manager.getOWLDataFactory();
    IRI ontologyIRI = ont.getOntologyID().getOntologyIRI().get();
    // Get class
    ...
    if (ont.containsClassInSignature(classIRI) == false) {
      throw new InstanceNotFoundException("The class \"" + obj.getClass().getSimpleName() + "\" was not found in the ontology");
    }
